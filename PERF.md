...
All under `perf/` (no external load tool, no npm install — Node built-ins
only):
...
```bash
perf/run_perf.sh --docs 20000 --doc-size 1024 --duration 15 \
...
- `perf/` — the scripts above.
- `PARITY.md` — feature-parity map and the always-on test harness.
- Query-engine memory notes in the repository `docs/` and the engine source
  (`crates/query/src/engine.rs`, `services.rs`).
